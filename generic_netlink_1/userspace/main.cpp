...
int on_response(struct nl_sock *sock, std::function<void(struct nlattr **attrs)> on_response)
{
	...
	nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM, [] (auto msg, auto arg) -> int {
		...
		auto fun = static_cast<std::function<void(struct nlattr **attrs)> *>(arg);
		(*fun)(attrs);
...
nla_put_failure:
	...
	ret = on_response(nl_sock, [&] (auto attrs) {
